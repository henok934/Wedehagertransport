...
class RoutesInsertView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            # 1. Validation: Source and Destination check
            ...
            # 2. Validation: Conflict checks
            if Route.objects.filter(side_no=side_no, date=route_date, plate_no=plate_no).exists():
                context['error'] = 'This bus is already reserved for this date.'
                return render(request, 'users/route.html', context)

            # 3. Save Primary Route
            serializer.save()

            # 4. Automatic Return Trip for Addisababa
            # We use str(depcity) to ensure we are comparing the name, not the object
            if str(depcity).strip() == "Addisababa":
                try:
                    # Ensure route_date is a date object for math
                    ...
                    # Catching the specific concatenation error or others
...
class Books(APIView):
    ...
    def get(self, request):
        ...
        return render(request, 'users/book.html', {
            ...
            'city': city,
            'buschanges_count': buschanges_count,
            'total_today': total_today # Sent to template
        })
        ...
        if 'text/html' in request.META.get('HTTP_ACCEPT', ''):
            ...
            return render(request, 'users/booker.html', {
                **response_data,
                ...
                'buschanges_count': buschanges_count, 'total_today': total_today
            })
        return Response(response_data, status=status.HTTP_200_OK)
...
class Workers(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            if Worker.objects.filter(username=username).exists():
                return render(request, 'users/worker.html', {'des': des, 'error': 'User name already exists.'})
                return Response({'error': 'User name already exists.'}, status=status.HTTP_400_BAD_REQUEST)

            if Worker.objects.filter(phone=phone).exists():
                return render(request, 'users/worker.html', {'des': des, 'error': 'Phone number already exists.'})
                return Response({'error': 'Phone number already exists.'}, status=status.HTTP_400_BAD_REQUEST)
            serializer.save()
            return render(request, 'users/worker.html', {'des': des, 'success': 'User registered successfully.'})
            return Response({'success': 'User registered successfully.'}, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class MyBus(generics.GenericAPIView):
    ...
    queryset = Bus.objects.all()
    serializer_class = BusSerializer # Use BusSerializer
    ...
    def get(self, request, *args, **kwargs):
        ...
        # Logic to assign 'buses' variable correctly in all paths
        if first_part == '3' or second_part == '3':
            ...
            # FIXED: Changed 'routes' to 'buses' here
            buses = Bus.objects.filter(filters)
            print(buses)
        # Handling both HTML and API responses
        if 'text/html' in request.META.get('HTTP_ACCEPT', ''):
            return render(request, 'users/mybus.html', {
                'name': sc_user.name,
                'side': side,
                'buses': buses # Raw QuerySet for template
            })
...
class BusInsertView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # 1. Retrieve the SC User from session
        sc_user = self.get_user_from_session(request)
        if not sc_user:
            return self.handle_error(request, 'User session not found.')

        # 2. Retrieve the level from Sc and add it to Bus level
        # Assuming your Sc model has a 'level' field
        data['level'] = getattr(sc_user, 'level', 'Level 1') 
        data['owner_sc'] = sc_user.id
        data['name'] = sc_user.name

        # 3. Clean and format the Plate No with ET-
        ...
        if serializer.is_valid():
            ...
            serializer.save()
            return self.handle_success(request, f'Bus {plate_no} registered successfully at {data["level"]}.')
            ...
            with transaction.atomic():
                for i in range(min_length):
                    ...
                    if already_booked:
                        ...
                        #error_msg = f"Person already booked: {firstnames[i]} {lastnames[i]} for {current_date} or {alt_date}."
                        if 'text/html' in request.META.get('HTTP_ACCEPT', ''):
                            error_context['error'] = error_msg
                            return render(request, 'users/ticket.html', error_context, status=400)
                        return Response({'error': error_msg}, status=400)
    ...
    def post(self, request):
        ...
        with transaction.atomic():
            for i in range(min_length):
                ...
                routes = Route.objects.filter(depcity=dep, descity=des, date=date, plate_no=plate)
                
                
                if not routes.exists():
                    error_msg = f"No route found for {dep} to {des} on {date}."
                    return render(request, 'users/ticket.html', {'des': City.objects.all(), 'error': error_msg}, status=400)
                ...
                total_seats = int(bus.no_seats)
                booked_in_db = Ticket.objects.filter(depcity=dep, descity=des, date=date, plate_no=plate).values_list('no_seat', flat=True)
                booked_seats_list = list(set(int(s) for s in booked_in_db if s))
                unbooked_seats = [s for s in range(1, total_seats + 1) if s not in booked_seats_list]
                ...
                if current_seat in used_seats:
                    if 'text/html' in request.META.get('HTTP_ACCEPT', ''):
                        return render(request, 'users/ticket.html', {'des': City.objects.all(), 'error': f'Seat {current_seat} already selected.', 'routes': routes}, error_context, status=400)
                ...
                if already_booked:
                    error_msg = f"Person already booked: {firstnames[i]} {lastnames[i]} for {current_date} or {alt_date}."
                    if 'text/html' in request.META.get('HTTP_ACCEPT', ''):
                        return render(request, 'users/ticket.html', {'des': City.objects.all(), 'error': error_msg, 'routes': routes, error_context}, status=400)
                    return Response({'error': error_msg}, status=400)
...
class Totalballance(APIView):
    # Link the class to the serializer to clear the "Unable to guess" errors
    ...
    def post(self, request):
        ...
        # 1. Sum up ticket prices by username
        totals_by_username = {}
        ...
        # 2. Get Worker details (City, Firstname, Lastname, Phone)
        workers = Worker.objects.filter(username__in=totals_by_username.keys())

        # Build a map of all worker info
        worker_info = {
            worker.username: {
                'city': worker.city,
                'fname': worker.fname,
        ...
        # 3. Combine everything into total_data
        total_data = {
            username: {
                'total_balance': total,
        ...
        if 'text/html' in request.META.get('HTTP_ACCEPT', ''):
            return render(request, 'users/totalballance.html', {'totals': total_data})
